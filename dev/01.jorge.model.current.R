#script to run maxent for jorge
...
#wd = "/home/jc140298/jorge/" #define the working directory - where to put the outputs
#species = "ABT"	#define the species of interest
occur.data = paste(wd, "occur.csv", sep="") #define the lon/lat of the observation records -- 2 column matrix of longitude and latitude
bkgd.data = "/home/jc140298/jorge_setup/bkgd.csv" #define the the lon/lat of the background / psuedo absence points to use -- 2 column matrix of longitude and latitude
enviro.data =  list.files("/home/jc140298/jorge_setup/ascii", pattern="\\.asc", recursive=TRUE, full.names=TRUE)
...
for (ii in 1:length(enviro.data)) { cat(ii,'of',length(enviro.data),'\n') #cycle through each of the environmental datasets and append the data
	...
	tonamecolumn = sub(".asc", "", basename(enviro.data[ii]))
 	occur[,tonamecolumn] = extract.data(cbind(occur$Longitude,occur$Latitude),tasc) #extract envirodata for observations
}
save(occur,file=paste(wd, "occur.RData", sep=""));
#bkgd = read.csv(bkgd.data) #read in the background position data lon.lat

## function needed for tryCatch'ing:
err.null <- function (e) return(NULL)

###run, store and project the models 
...
if (project.maxent) {
	outdir = paste(wd,'output_maxent/',sep='');
	tstr = paste("java -cp ", maxent.jar, " density.Project ", outdir, species, ".lambdas ", sep="")
	# where to find the climate scenarios
	tstr = paste(tstr, dirname(enviro.data[1]), " ", sep="")
	# where to put, what to name the output
	tstr = paste(tstr, wd, species, ".asc", sep="")
	# optional arguments
